% vip_microcircuit_feedback_test.m
clear; clc; close all
% This network model was constructed based on Izhikevich neural network
% Create by Guangwei Xu
% =======================
% Test feedback inhibition by regulating FB-Inh intrinsic excitability
% =======================
...
% Define FB-Inh excitability gradient
fb_excitability_levels = 1:0.1:2.5;
num_repeats = 50;
% analyze_inhibitory_population_effects
% Run FB-Inh excitability gradient experiment
[detailed_results] = run_fb_excitability_gradient(S_raw, net, fb_excitability_levels, num_repeats);

% Analyze and visualize results
analyze_fb_excitability_results(detailed_results, fb_excitability_levels);
create_fb_excitability_figure_with_errorbars(detailed_results, fb_excitability_levels, net);
analyze_synchrony_results(detailed_results, fb_excitability_levels);
create_synchrony_analysis_figure_with_errorbars(detailed_results, fb_excitability_levels);

% Save data
save('temp_results.mat', 'detailed_results', 'fb_excitability_levels');
...
visualize_common_input_analysis_with_complete_errorbars(improved_results, fb_excitability_levels, detailed_results);
% =======================
% Core function definitions
% =======================
function visualize_common_input_analysis_with_complete_errorbars(improved_results, fb_excitability_levels, detailed_results)
% Add complete error bars for common input analysis

figure('Position', [100, 100, 1400, 800]);

% Calculate standard deviations for each metric
n_conditions = length(fb_excitability_levels);
...
subplot(2, 3, 1);
errorbar(fb_excitability_levels, improved_results.temporal_variability_truncated, temporal_var_truncated_std, 'o-', 'LineWidth', 2);
xlabel('FB-Inh Excitability');
ylabel('Common Input Strength (Truncation Method)');
title('A. Temporal Variability - Truncation Method');
grid on;

subplot(2, 3, 2);
errorbar(fb_excitability_levels, improved_results.temporal_variability_interpolated, temporal_var_interpolated_std, 's-', 'LineWidth', 2);
xlabel('FB-Inh Excitability');
...
subplot(2, 3, 3);
errorbar(fb_excitability_levels, improved_results.mean_trial_correlation, trial_correlation_std, '^-', 'LineWidth', 2);
xlabel('FB-Inh Excitability');
...
subplot(2, 3, 4);
errorbar(fb_excitability_levels, improved_results.pca_commonality, pca_commonality_std, 'd-', 'LineWidth', 2);
xlabel('FB-Inh Excitability');
...
subplot(2, 3, 5);
errorbar(fb_excitability_levels, improved_results.composite_strength, composite_strength_std, 'p-', 'LineWidth', 2);
xlabel('FB-Inh Excitability');
...
errorbar(fb_excitability_levels, lengths_per_condition, lengths_std, 'v-', 'LineWidth', 2);
xlabel('FB-Inh Excitability');
ylabel('Mean Sequence Length');
title('F. Time Series Length');
grid on;

sgtitle('Analysis of FB-Inh Excitability Effects on Common Input with Variable-Length Time Series (with Error Bars)', 'FontSize', 16);
end

function create_fb_excitability_figure_with_errorbars(detailed_results, fb_excitability_levels, net)
% Create comprehensive analysis figure for FB-Inh excitability experiment - add all missing error bars
...
mean_fb_efficacy = mean(detailed_results.fb_efficacy, 2);
std_fb_efficacy = std(detailed_results.fb_efficacy, 0, 2);
...
subplot(2, 4, 1);
errorbar(fb_excitability_levels, mean_exc3, std_exc3, 'b-o', 'LineWidth', 2, 'MarkerSize', 8);
xlabel('FB-Inh Excitability Level');
ylabel('Exc3 Firing Rate (Hz)');
title('A. Exc3 Activity vs FB-Inh Excitability');
grid on;

if length(fb_excitability_levels) > 2
    p = polyfit(fb_excitability_levels, mean_exc3', 1);
    x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
    hold on;
...
subplot(2, 4, 2);
errorbar(fb_excitability_levels, mean_fb_inh, std_fb_inh, 'm-s', 'LineWidth', 2, 'MarkerSize', 8);
xlabel('FB-Inh Excitability Level');
ylabel('FB-Inh Firing Rate (Hz)');
title('B. FB-Inh Activity vs Excitability');
grid on;

subplot(2, 4, 3);
errorbar(fb_excitability_levels, mean_noise_corr, std_noise_corr, 'r-^', 'LineWidth', 2, 'MarkerSize', 8);
xlabel('FB-Inh Excitability Level');
ylabel('Noise Correlation');
title('C. Noise Correlation vs FB-Inh Excitability (Key)');
grid on;

valid_idx = ~isnan(mean_noise_corr);
if sum(valid_idx) > 2
    p = polyfit(fb_excitability_levels(valid_idx), mean_noise_corr(valid_idx)', 1);
    x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
    hold on;
...
% 4. Modified: Feedback inhibition efficacy - add error bars
subplot(2, 4, 4);
errorbar(fb_excitability_levels, mean_fb_efficacy, std_fb_efficacy, 'g-d', 'LineWidth', 2, 'MarkerSize', 8);
xlabel('FB-Inh Excitability Level');
ylabel('Feedback Inhibition Efficacy');
title('D. Feedback Inhibition Efficacy');
grid on;

% 5-6. Keep scatter plots as is (scatter plots don't need errorbar)
subplot(2, 4, 5);
scatter(mean_fb_inh, mean_exc3, 100, fb_excitability_levels, 'filled', 'MarkerEdgeColor', 'black');
colorbar;
xlabel('FB-Inh Firing Rate (Hz)');
ylabel('Exc3 Firing Rate (Hz)');
title('E. Exc3 vs FB-Inh Activity');

if length(mean_fb_inh) > 2
    p = polyfit(mean_fb_inh, mean_exc3, 1);
    x_fit = linspace(min(mean_fb_inh), max(mean_fb_inh), 100);
    hold on;
    plot(x_fit, polyval(p, x_fit), 'k--', 'LineWidth', 2);
    
    [r, p_val] = corrcoef(mean_fb_inh, mean_exc3);
...
subplot(2, 4, 6);
valid_idx = ~isnan(mean_noise_corr);
scatter(mean_fb_inh(valid_idx), mean_noise_corr(valid_idx), 100, fb_excitability_levels(valid_idx), 'filled', 'MarkerEdgeColor', 'black');
colorbar;
xlabel('FB-Inh Firing Rate (Hz)');
ylabel('Noise Correlation');
title('F. Noise Correlation vs FB-Inh Activity');
...
subplot(2, 4, 7);
errorbar(fb_excitability_levels, mean_variability, std_variability, 'c-p', 'LineWidth', 2, 'MarkerSize', 8);
xlabel('FB-Inh Excitability Level');
...
norm_fb_inh = (mean_fb_inh - min(mean_fb_inh)) / (max(mean_fb_inh) - min(mean_fb_inh));
...
norm_std_fb_inh = std_fb_inh / (max(mean_fb_inh) - min(mean_fb_inh));
if length(valid_noise) > 1
...
h1 = errorbar(fb_excitability_levels, norm_exc3, norm_std_exc3, 'b-o', 'LineWidth', 2); 
hold on;
h2 = errorbar(fb_excitability_levels, norm_fb_inh, norm_std_fb_inh, 'm-s', 'LineWidth', 2);

valid_noise_idx = ~isnan(norm_noise);
if sum(valid_noise_idx) > 0
    h3 = errorbar(fb_excitability_levels(valid_noise_idx), norm_noise(valid_noise_idx), norm_std_noise(valid_noise_idx), 'r-^', 'LineWidth', 2);
    legend([h1, h2, h3], {'Normalized Exc3', 'Normalized FB-Inh', 'Normalized Noise Correlation'}, 'Location', 'best');
else
    legend([h1, h2], {'Normalized Exc3', 'Normalized FB-Inh'}, 'Location', 'best');
end
...
sgtitle('FB-Inh Excitability Gradient Experiment: Feedback Inhibition Mechanism Analysis (Complete Error Bar Version)', 'FontSize', 16, 'FontWeight', 'bold');

% Save figures
saveas(gcf, 'fb_excitability_analysis_with_errorbars.png');
saveas(gcf, 'fb_excitability_analysis_with_errorbars.fig');
end

function create_synchrony_analysis_figure_with_errorbars(detailed_results, fb_excitability_levels)
...
fb_excitability_levels = fb_excitability_levels(:);

% 1-7. Keep first seven subplots as is (already have errorbar)
subplot(2, 4, 1);
valid_idx = ~isnan(mean_plv);
if sum(valid_idx) > 0
    errorbar(fb_excitability_levels(valid_idx), mean_plv(valid_idx), std_plv(valid_idx), 'b-o', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('FB-Inh Excitability Level');
    ylabel('Phase Locking Value (PLV)');
    title('A. Phase Locking vs FB-Inh Excitability');
    grid on;
    
    if sum(valid_idx) > 2
        p = polyfit(fb_excitability_levels(valid_idx), mean_plv(valid_idx), 1);
        x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
        hold on;
        plot(x_fit, polyval(p, x_fit), 'b--', 'LineWidth', 2);
        
        [r, p_val] = corrcoef(fb_excitability_levels(valid_idx), mean_plv(valid_idx));
        text(0.1, 0.9, sprintf('r = %.3f\np = %.3f', r(1,2), p_val(1,2)), ...
             'Units', 'normalized', 'BackgroundColor', 'white', 'FontSize', 10);
    end
end
...
if sum(valid_idx) > 0
    errorbar(fb_excitability_levels(valid_idx), mean_stv(valid_idx), std_stv(valid_idx), 'r-s', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('FB-Inh Excitability Level');
    ...
    if sum(valid_idx) > 2
        p = polyfit(fb_excitability_levels(valid_idx), mean_stv(valid_idx), 1);
        x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
        hold on;
        plot(x_fit, polyval(p, x_fit), 'r--', 'LineWidth', 2);
        
        [r, p_val] = corrcoef(fb_excitability_levels(valid_idx), mean_stv(valid_idx));
        text(0.1, 0.9, sprintf('r = %.3f\np = %.3f', r(1,2), p_val(1,2)), ...
             'Units', 'normalized', 'BackgroundColor', 'white', 'FontSize', 10);
    end
end
...
if sum(valid_idx) > 0
    errorbar(fb_excitability_levels(valid_idx), mean_isi(valid_idx), std_isi(valid_idx), 'g-^', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('FB-Inh Excitability Level');
    ...
    if sum(valid_idx) > 2
        p = polyfit(fb_excitability_levels(valid_idx), mean_isi(valid_idx), 1);
        x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
        hold on;
        plot(x_fit, polyval(p, x_fit), 'g--', 'LineWidth', 2);
        
        [r, p_val] = corrcoef(fb_excitability_levels(valid_idx), mean_isi(valid_idx));
        text(0.1, 0.9, sprintf('r = %.3f\np = %.3f', r(1,2), p_val(1,2)), ...
             'Units', 'normalized', 'BackgroundColor', 'white', 'FontSize', 10);
    end
end
...
if sum(valid_idx) > 0
    errorbar(fb_excitability_levels(valid_idx), mean_vs(valid_idx), std_vs(valid_idx), 'm-d', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('FB-Inh Excitability Level');
    ...
    if sum(valid_idx) > 2
        p = polyfit(fb_excitability_levels(valid_idx), mean_vs(valid_idx), 1);
        x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
        hold on;
        plot(x_fit, polyval(p, x_fit), 'm--', 'LineWidth', 2);
        
        [r, p_val] = corrcoef(fb_excitability_levels(valid_idx), mean_vs(valid_idx));
        text(0.1, 0.9, sprintf('r = %.3f\np = %.3f', r(1,2), p_val(1,2)), ...
             'Units', 'normalized', 'BackgroundColor', 'white', 'FontSize', 10);
    end
end
...
if sum(valid_idx) > 0
    errorbar(fb_excitability_levels(valid_idx), mean_pc(valid_idx), std_pc(valid_idx), 'c-p', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('FB-Inh Excitability Level');
    ...
    if sum(valid_idx) > 2
        p = polyfit(fb_excitability_levels(valid_idx), mean_pc(valid_idx), 1);
        x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
        hold on;
        plot(x_fit, polyval(p, x_fit), 'c--', 'LineWidth', 2);
        
        [r, p_val] = corrcoef(fb_excitability_levels(valid_idx), mean_pc(valid_idx));
        text(0.1, 0.9, sprintf('r = %.3f\np = %.3f', r(1,2), p_val(1,2)), ...
             'Units', 'normalized', 'BackgroundColor', 'white', 'FontSize', 10);
    end
end
...
if sum(valid_idx) > 0
    errorbar(fb_excitability_levels(valid_idx), mean_pb(valid_idx), std_pb(valid_idx), 'k-h', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('FB-Inh Excitability Level');
    ...
    if sum(valid_idx) > 2
        p = polyfit(fb_excitability_levels(valid_idx), mean_pb(valid_idx), 1);
        x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
        hold on;
        plot(x_fit, polyval(p, x_fit), 'k--', 'LineWidth', 2);
        
        [r, p_val] = corrcoef(fb_excitability_levels(valid_idx), mean_pb(valid_idx));
        text(0.1, 0.9, sprintf('r = %.3f\np = %.3f', r(1,2), p_val(1,2)), ...
             'Units', 'normalized', 'BackgroundColor', 'white', 'FontSize', 10);
    end
end
...
if ~isempty(sync_matrix)
    ...
    colormap(jet);
    colorbar;
    set(gca, 'XTick', 1:length(fb_excitability_levels));
    set(gca, 'XTickLabel', arrayfun(@(x) sprintf('%.1f', x), fb_excitability_levels, 'UniformOutput', false));
...
subplot(2, 4, 8);
n_levels = length(fb_excitability_levels);
...
if sum(valid_idx) > 0
    ...
    errorbar(fb_excitability_levels(valid_idx), comprehensive_sync(valid_idx), comprehensive_sync_std(valid_idx), ...
    ...
    % Add trend line
    if sum(valid_idx) > 2
        p = polyfit(fb_excitability_levels(valid_idx), comprehensive_sync(valid_idx), 1);
        x_fit = linspace(min(fb_excitability_levels), max(fb_excitability_levels), 100);
        hold on;
        plot(x_fit, polyval(p, x_fit), '--', 'LineWidth', 2, 'Color', [0.5, 0.5, 0.5]);
        
        [r, p_val] = corrcoef(fb_excitability_levels(valid_idx), comprehensive_sync(valid_idx));
        ...
        if r(1,2) < -0.3 && p_val(1,2) < 0.1
            text(0.1, 0.7, '✓ FB-Inh Reduces Synchrony', 'Units', 'normalized', ...
                 'BackgroundColor', 'green', 'FontSize', 10);
        elseif r(1,2) > 0.3 && p_val(1,2) < 0.1
            text(0.1, 0.7, '✗ FB-Inh Increases Synchrony', 'Units', 'normalized', ...
                 'BackgroundColor', 'red', 'FontSize', 10);
        else
            text(0.1, 0.7, '? Trend Not Clear', 'Units', 'normalized', ...
                 'BackgroundColor', 'yellow', 'FontSize', 10);
        end
    end
end

sgtitle('Analysis of FB-Inh Excitability Effects on Neural Network Synchrony (Complete Error Bar Version)', 'FontSize', 16, 'FontWeight', 'bold');
...
function [detailed_results,analysis_data] = run_fb_excitability_gradient(S_raw, net, fb_excitability_levels, num_repeats)
% Run FB-Inh excitability gradient experiment (add synchrony analysis)
...
detailed_results.fb_efficacy = zeros(n_conditions, num_repeats);
...
fprintf('Running FB-Inh excitability gradient experiment with synchrony analysis...\n');

for cond_idx = 1:n_conditions
    fb_excitability = fb_excitability_levels(cond_idx);
    fprintf('FB-Inh Excitability Level: %.1f\n', fb_excitability);
    ...
    for repeat = 1:num_repeats
        ...
        condition = struct(...
            'name', sprintf('FB_Excitability_%.1f', fb_excitability), ...
            'input_layer1', 5, ...
            'input_layer2', 4, ...
            'input_layer3', 4, ...
            'vip_activation', 1.0, ...
            'fb_excitability', fb_excitability, ...
            'ff_strength', 1.0, ...
            'fb_strength', 1.0);
        
        % Run simulation
        [firings, rates, spike_times, analysis_data] = ...
            run_fb_excitability_simulation(v, u, a, b, c, d, S_raw, N, net, condition);
        ...
        fb_efficacy = calculate_feedback_efficacy_detailed(rates, net);
        detailed_results.fb_efficacy(cond_idx, repeat) = fb_efficacy;
        ...
        detailed_results.effective_common_input(cond_idx, repeat) = ...
            calculate_effective_common_input_single(analysis_data);
    ...
    % if cond_idx == 1 || cond_idx == length(fb_excitability_levels)
    %     fprintf('FB excitability%.1f: Average FB-Inh input=%.2f, Average Exc3 input=%.2f\n', ...
    %         fb_excitability, mean(I(net.inh3_fb)), mean(I(net.exc3)));
    % end
end
analyze_fb_connectivity(S_raw, net)
fprintf('Experiment completed with detailed synchrony analysis.\n');
end
function analyze_synchrony_results(detailed_results, fb_excitability_levels)
...
fprintf('\nFB Excit\tPhase Lock\t\tSpike Var\t\tISI Sync\t\tVector Str\t\tPairwise\t\tPop Burst\n');
fprintf('--------------------------------------------------------------------------------------------------\n');
for i = 1:length(fb_excitability_levels)
    fprintf('%.1f\t\t%.4f\t\t\t%.4f\t\t\t%.4f\t\t\t%.4f\t\t\t%.4f\t\t\t%.4f\n', ...
        fb_excitability_levels(i), ...
        mean_plv(i), mean_stv(i), mean_isi(i), ...
        mean_vs(i), mean_pc(i), mean_pb(i));
end

% Correlation analysis
fprintf('\n=== Correlation of Synchrony Metrics with FB-Inh Excitability ===\n');
valid_idx = ~isnan(mean_plv);
if sum(valid_idx) > 2
    [r_plv, p_plv] = corrcoef(fb_excitability_levels(valid_idx)', mean_plv(valid_idx));
    fprintf('Phase locking value: r = %.3f, p = %.3f\n', r_plv(1,2), p_plv(1,2));
end

valid_idx = ~isnan(mean_vs);
if sum(valid_idx) > 2
    [r_vs, p_vs] = corrcoef(fb_excitability_levels(valid_idx)', mean_vs(valid_idx));
    fprintf('Vector strength: r = %.3f, p = %.3f\n', r_vs(1,2), p_vs(1,2));
end

valid_idx = ~isnan(mean_pb);
if sum(valid_idx) > 2
    [r_pb, p_pb] = corrcoef(fb_excitability_levels(valid_idx)', mean_pb(valid_idx));
...
function [firings, rates, spike_times, analysis_data] = ...
    run_fb_excitability_simulation(v, u, a, b, c, d, S_raw, N, net, condition)
% Run FB-Inh excitability simulation (add synchrony analysis)
...
for t = 1:T
    % Calculate input
    % I = calculate_improved_fb_excitability_input(t, N, noise_streams(:,t), noise_scale, net, condition);
    [I, common_input] = calculate_fb_excitability_input_with_common(t, length(v), randn(length(v), 1), 3, net, condition);
...
% analysis_data.fb_inh_spike_times = fb_inh_spike_times;
...
function analyze_fb_excitability_results(detailed_results, fb_excitability_levels)
% Analyze FB-Inh excitability experiment results

fprintf('\n=== FB-Inh Excitability Gradient Experiment Analysis ===\n');
...
% Output results table
fprintf('\nFB Excit\tExc3 Rate\t\tFB-Inh Rate\tNoise Corr\t\tFB Efficacy\t\tExc3 Variability\n');
fprintf('-----------------------------------------------------------------------------------------\n');
for i = 1:length(fb_excitability_levels)
...
% FB-Inh activity vs Exc3 activity
[r_fb_exc3, p_fb_exc3] = corrcoef(mean_fb_inh, mean_exc3);
fprintf('FB-Inh activity vs Exc3 activity: r = %.3f, p = %.3f\n', r_fb_exc3(1,2), p_fb_exc3(1,2));

% FB-Inh activity vs Noise correlation
valid_idx = ~isnan(mean_noise_corr);
if sum(valid_idx) > 2
    [r_fb_noise, p_fb_noise] = corrcoef(mean_fb_inh(valid_idx), mean_noise_corr(valid_idx));
    fprintf('FB-Inh activity vs Noise correlation: r = %.3f, p = %.3f\n', r_fb_noise(1,2), p_fb_noise(1,2));
end
...
if length(fb_excitability_levels) > 2
    ...
    p_exc3 = polyfit(fb_excitability_levels, mean_exc3', 1);
    fprintf('Exc3 firing rate slope: %.3f Hz/unit FB excitability\n', p_exc3(1));
    
    % Noise correlation trend
    if sum(valid_idx) > 2
        p_noise = polyfit(fb_excitability_levels(valid_idx), mean_noise_corr(valid_idx)', 1);
        fprintf('Noise correlation slope: %.4f /unit FB excitability\n', p_noise(1));
        
        if p_noise(1) < 0
            fprintf('✓ Conclusion: Increased FB-Inh excitability leads to decreased noise correlation\n');
        else
            fprintf('✗ Anomaly: Increased FB-Inh excitability leads to increased noise correlation\n');
        end
    end
end

% Feedback inhibition efficacy analysis
max_efficacy_idx = find(mean_fb_efficacy == max(mean_fb_efficacy), 1);
fprintf('\nOptimal feedback inhibition efficacy at FB excitability = %.1f, efficacy = %.3f\n', ...
    fb_excitability_levels(max_efficacy_idx), max(mean_fb_efficacy));
end
...
prob_ff_inhibition = 0.6;    % Reduced from 0.7 to 0.6
...
% ========================
% *** Key modification 3: E/I balance-based feedback inhibition design ***
% ========================
...
for i = inh3_fb
    for j = vip
        if rand() <= prob_vip_disinhibition
            S_raw(i, j) = vip_weight()*0.25;  % Reduced from 0.3 to 0.25
        end
    end
end
...
for i = inh3_ff
    for j = vip
        if rand() <= prob_vip_disinhibition * 0.2  % Reduced from 0.3 to 0.2
            S_raw(i, j) = vip_weight() * 0.3;  % Reduced from 0.4 to 0.3
        end
    end
end
...
function analyze_fb_connectivity(S_raw, net)
% Detailed analysis of FB neuron connectivity structure

fprintf('\n=== Detailed FB Neuron Connectivity Structure Analysis ===\n');
...
% 1. Analyze Exc3 → FB-Inh connections
fprintf('\n1. Exc3 → FB-Inh Activation Connections:\n');
exc_to_fb_connections = 0;
exc_to_fb_weights = [];

for i = net.inh3_fb
    for j = net.exc3
        if S_raw(i, j) > 0  % Excitatory connection
            exc_to_fb_connections = exc_to_fb_connections + 1;
            exc_to_fb_weights = [exc_to_fb_weights, S_raw(i, j)];
        end
    end
end

fprintf('  Number of connections: %d/%d (%.1f%%)\n', exc_to_fb_connections, length(net.exc3)*length(net.inh3_fb), ...
        100*exc_to_fb_connections/(length(net.exc3)*length(net.inh3_fb)));
if ~isempty(exc_to_fb_weights)
    fprintf('  Connection weights: %.2f ± %.2f (range: %.2f-%.2f)\n', ...
            mean(exc_to_fb_weights), std(exc_to_fb_weights), ...
            min(exc_to_fb_weights), max(exc_to_fb_weights));
end

% 2. Analyze FB-Inh → Exc3 connections (key inhibitory connections)
fprintf('\n2. FB-Inh → Exc3 Inhibition Connections:\n');
fb_to_exc_connections = 0;
fb_to_exc_weights = [];
connection_matrix = zeros(length(net.exc3), length(net.inh3_fb));

for i = net.exc3
    for j = net.inh3_fb
        if S_raw(i, j) < 0  % Inhibitory connection
            fb_to_exc_connections = fb_to_exc_connections + 1;
            fb_to_exc_weights = [fb_to_exc_weights, abs(S_raw(i, j))];
            
            % Record connection matrix
            exc_idx = find(net.exc3 == i);
            fb_idx = find(net.inh3_fb == j);
            connection_matrix(exc_idx, fb_idx) = abs(S_raw(i, j));
        end
    end
end

fprintf('  Number of connections: %d/%d (%.1f%%)\n', fb_to_exc_connections, length(net.exc3)*length(net.inh3_fb), ...
        100*fb_to_exc_connections/(length(net.exc3)*length(net.inh3_fb)));
if ~isempty(fb_to_exc_weights)
    fprintf('  Inhibition weights: %.2f ± %.2f (range: %.2f-%.2f)\n', ...
            mean(fb_to_exc_weights), std(fb_to_exc_weights), ...
            min(fb_to_exc_weights), max(fb_to_exc_weights));
end

% 3. Display connection matrix
fprintf('\n3. FB-Inh → Exc3 Connection Matrix (rows=Exc3, columns=FB-Inh):\n');
fprintf('    ');
for j = 1:length(net.inh3_fb)
    fprintf('FB%d   ', j);
end
fprintf('\n');
...
% 4. Analyze influence range of each FB-Inh neuron
fprintf('\n4. Influence Analysis of Each FB-Inh Neuron:\n');
for fb_idx = 1:length(net.inh3_fb)
    fb_neuron = net.inh3_fb(fb_idx);
    targets = [];
    weights = [];
    
    for exc_neuron = net.exc3
        if S_raw(exc_neuron, fb_neuron) < 0
            targets = [targets, exc_neuron];
            weights = [weights, abs(S_raw(exc_neuron, fb_neuron))];
        end
    end
    
    fprintf('  FB-Inh_%d (neuron#%d): Inhibits %d Exc3 neurons', fb_idx, fb_neuron, length(targets));
    if ~isempty(weights)
...
% 5. Analyze FB inhibition received by each Exc3 neuron
fprintf('\n5. FB Inhibition Analysis Received by Each Exc3 Neuron:\n');
fb_inhibition_per_exc = zeros(length(net.exc3), 1);
for exc_idx = 1:length(net.exc3)
    exc_neuron = net.exc3(exc_idx);
    total_fb_inhibition = 0;
    fb_sources = 0;
    
    for fb_neuron = net.inh3_fb
        if S_raw(exc_neuron, fb_neuron) < 0
            total_fb_inhibition = total_fb_inhibition + abs(S_raw(exc_neuron, fb_neuron));
            fb_sources = fb_sources + 1;
        end
    end
    
    fb_inhibition_per_exc(exc_idx) = total_fb_inhibition;
    
    if exc_idx <= 5  % Only show first 5
        fprintf('  Exc3_%d: Total FB inhibition=%.1f (from %d FB-Inh)\n', ...
                exc_idx, total_fb_inhibition, fb_sources);
    end
end

fprintf('  FB inhibition distribution: %.1f±%.1f (range: %.1f-%.1f)\n', ...
        mean(fb_inhibition_per_exc), std(fb_inhibition_per_exc), ...
        min(fb_inhibition_per_exc), max(fb_inhibition_per_exc));

% 6. Lateral inhibition among FB-Inh neurons
fprintf('\n6. Lateral Inhibition Among FB-Inh Neurons:\n');
...
for i = net.inh3_fb
    for j = net.inh3_fb
        if i ~= j && S_raw(i, j) < 0
            lateral_connections = lateral_connections + 1;
            lateral_weights = [lateral_weights, abs(S_raw(i, j))];
        end
    end
end

fprintf('  Lateral inhibition connections: %d/%d\n', lateral_connections, length(net.inh3_fb)*(length(net.inh3_fb)-1));
if ~isempty(lateral_weights)
    fprintf('  Lateral inhibition weights: %.1f±%.1f\n', mean(lateral_weights), std(lateral_weights));
end

fprintf('\n=== FB Connection Analysis Complete ===\n');
end
...
function [I, common_input] = calculate_fb_excitability_input_with_common(t, N, noise, noise_scale, net, condition)
% Calculate input and return common input strength

% Base input
I = 0.5 + 0.2 * randn(N, 1);  % Reduced from 1+0.3
...
if condition.input_layer3 > 0
    ...
    I(net.exc3) = I(net.exc3) + condition.input_layer3*0.6 + common_input * 0.15;  % Maintain strong input, add appropriate common input
end
...
if isfield(condition, 'fb_excitability')
    ...
    enhanced_drive = fb_drive * (1 + 0.8 * fb_drive);  % Increased from 0.3 to 0.8
    nfb = length(net.inh3_fb);
    
    % More sensitive activation
    for k = 1:length(net.inh3_fb)
        neuron_idx = net.inh3_fb(k);
        threshold = 0.1 + 0.2 * (k-1)/max(1,nfb-1);  % Lower threshold
        gain      = 2.0 + 1.5 * (k-1)/max(1,nfb-1);  % Higher gain
...
% Keep VIP in inhibitory state
I(net.inh1) = I(net.inh1) - 0.3;
I(net.inh2) = I(net.inh2) - 0.3;
I([net.inh3_ff]) = I([net.inh3_ff]) - 0.2;
...
function effective_input = calculate_effective_common_input_single(analysis_data)
% Calculate effective common input for single trial
...
if isfield(analysis_data, 'common_input_series') && ~isempty(analysis_data.common_input_series)
    ...
    [common_input_std,common_input_mean] = calculate_common_input_std(analysis_data)
    effective_input = common_input_std / common_input_mean;
else
    effective_input = NaN;
end
end
...
function [common_input_std, common_input_mean] = calculate_common_input_std(analysis_data)
% Calculate standard deviation of common input for each condition

[n_conditions, n_repeats] = size(analysis_data.common_input_series);
common_input_std = zeros(n_conditions, 1);
common_input_mean = zeros(n_conditions, 1);
for i = 1:n_conditions
    all_values = [];
    
    % Collect data from all repeats for this condition
    for j = 1:n_repeats
        if ~isempty(analysis_data.common_input_series(i, j))
            series = analysis_data.common_input_series(i, j);
            all_values = [all_values, series(:)']; % Flatten and merge
        end
    end
    
    % Calculate overall standard deviation for this condition
    if ~isempty(all_values)
        common_input_std(i) = std(all_values);
        common_input_mean(i) = mean(all_values);
    else
        common_input_std(i) = NaN;
        common_input_mean(i) = NaN;
    end
end
end
...
% Process common input strength calculation for variable-length time series - fix interpolation error
...
for cond = 1:n_conditions
    ...
    for trial = 1:n_trials
        if ~isempty(detailed_results.exc3_time_series{cond, trial})
            ...
            if size(exc3_data, 1) == 1
                ...
                % *** Fix: Check sequence length ***
                if length(activity) >= 2  % Only accept sequences with length at least 2
                    trial_activities{end+1} = activity;
                    trial_lengths = [trial_lengths, length(activity)];
                    fprintf('  Trial %d: length %d\n', trial, length(activity));
                else
                    fprintf('  Trial %d: too short (%d), skipped\n', trial, length(activity));
                end
            end
        end
    end
    ...
    if length(trial_activities) >= 2  % At least 2 trials
        ...
        % Method 2: Interpolate to same length - fixed version
        ...
        for t = 1:length(trial_activities)
            ...
            % *** Fix: Check original sequence length ***
            if original_length == 1
                % Single-point sequence: extend as constant sequence
                interpolated_activities(t, :) = repmat(original, 1, max_length);
                fprintf('    Trial %d: single-point sequence, extended as constant\n', t);
            elseif original_length >= 2
                % Normal interpolation
                original_time = linspace(0, 1, original_length);
                new_time = linspace(0, 1, max_length);
                try
                    interpolated_activities(t, :) = interp1(original_time, original, new_time, 'linear');
                catch ME
                    % Backup plan when interpolation fails
                    fprintf('    Trial %d: interpolation failed, using repeat fill\n', t);
                    interpolated_activities(t, :) = repmat(original(1), 1, max_length);
                end
            end
        end
        ...
        % Method 3: Calculate inter-trial consistency (based on correlation) - fixed version
        trial_correlations = [];
        for t1 = 1:length(trial_activities)
            for t2 = t1+1:length(trial_activities)
                ...
                % *** Fix: Ensure enough points for correlation calculation ***
                if len >= 5  % At least 5 points to calculate correlation
                    seq1 = trial_activities{t1}(1:len);
                    seq2 = trial_activities{t2}(1:len);
                    
                    % Check if sequences have variability
                    if std(seq1) > eps && std(seq2) > eps
                        try
                            corr_matrix = corrcoef(seq1, seq2);
                            if size(corr_matrix, 1) == 2 && size(corr_matrix, 2) == 2
                                trial_correlations = [trial_correlations, corr_matrix(1,2)];
                            end
                        catch
                            % Skip when correlation calculation fails
                            fprintf('    Trial %d-%d: correlation calculation failed\n', t1, t2);
                        end
                    else
                        fprintf('    Trial %d-%d: sequences lack variability, skipped correlation calculation\n', t1, t2);
                    end
                else
                    fprintf('    Trial %d-%d: insufficient overlap length (%d < 5)\n', t1, t2, len);
                end
            end
        end
        
        if ~isempty(trial_correlations)
            mean_trial_correlation = mean(trial_correlations(~isnan(trial_correlations)));
        else
            mean_trial_correlation = NaN;
        end
        
        % Method 4: Analysis based on overlap interval - fixed version
        overlap_start = 1;
        overlap_end = min_length;
        if overlap_end >= 10 && length(trial_activities) >= 3  % At least 10 overlap points and 3 trials
            ...
            try
                ...
                if any(std(overlap_activities, 0, 2) > eps)
                    [coeff, score, latent] = pca(overlap_activities);
                    first_pc_variance = latent(1) / sum(latent);
                else
                    first_pc_variance = NaN;
                    fprintf('    Overlap interval lacks variability, skipped PCA\n');
                end
            catch
                first_pc_variance = NaN;
                fprintf('    PCA calculation failed\n');
            end
        else
            first_pc_variance = NaN;
            fprintf('    Insufficient overlap interval or trial count, skipped PCA\n');
        end
        ...
        % Composite metric - fixed version
        if ~isnan(mean_trial_correlation)
            improved_common_input_analysis.composite_strength(cond) = ...
                temporal_variability_truncated * (1 + mean_trial_correlation);
        else
            improved_common_input_analysis.composite_strength(cond) = temporal_variability_truncated;
        end
...
if ~isempty(all_lengths)
    ...
    % Count short sequences
    short_sequences = sum(all_lengths < 5);
    fprintf('Number of short sequences (<5 points): %d\n', short_sequences);
else
    fprintf('No valid time series data\n');
end
end